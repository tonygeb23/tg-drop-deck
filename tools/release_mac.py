...
MAC_NOTES = {
    "3.8.0": 'Everything the Windows copy gained between 3.6.0 and 3.7.1, and one thing it has not got: a virtual audio cable of Drop Deck\'s own. On air, Drop Deck Audio installs it, and after that it is in every program\'s microphone list under that name, with the send already pointed at it. TeamTalk, Zoom, Skype or OBS take Drop Deck Audio as their microphone and they get the whole show. Option+Shift+O sends it: pads, beds, running order, your microphone with its processing and every source you are catching, whether or not you are on air. It can leave one source out, so sending to a program you are also capturing does not hand that program its own audio back and fill the call with echo. Command+Shift+H puts the send in your own headphones, Command+Shift+O says whether it is arriving cleanly, and Command+Shift+W reads out where every single thing is going in one go. Command+Shift+R records the picture and the sound together into one MP4, off air as well as on, with the sound as the master clock. Both recording keys now write a .cue track list beside the recording for Mixcloud, and both say what is really in the file when they start. Command+Shift+C shows what is coming up from the ticked items in the running order. A source can be held back in milliseconds to line a capture card up with the others. And three faults went with it: Hear yourself through was saved and read by nothing, so monitoring always came out of the main card; opening a board threw away every video setting it had just loaded, so you kept the previous board\'s picture and destination; and F1 was eight keys behind, missing everything the video work added. Proved with a real send into the real cable, recorded off the far end: no gaps, a 1 kHz tone back at 1000.0 Hz at both 44100 and 48000, and silence when nothing is playing.',
    "3.5.28": 'Two things a source could still get away with. A source set to capture a program that is not running now says so before you go live. From macOS 26 the capture for a program can be made before that program starts, and waits for it, which is the right behaviour and also a brand new way for a source to pass every check and send nothing at all: the capture is fine, the program simply is not there. And the channel popup no longer sits dimmed on both channels mixed together for a captured program. Dimmed does not read as does not apply to somebody hearing it aloud, it reads as the setting in force, and it is the opposite of what happens: a program is always kept in stereo. It says stereo now, and the choice a source has as a device is left untouched underneath so switching back brings the real one straight back.',
    "3.5.27": "A captured program goes out in stereo. It was going out in mono, all of it, and the panel would not let you change that because the channel popup is switched off for a program. The board's channel setting is meant for a device, where a microphone on one leg of a stereo interface has to be mixed or picked, and its default is both channels mixed together. That was being applied to captured programs as well, so a stereo mix arrived flat. The tap is already a stereo mixdown of the program's own output, so there is nothing to choose and nothing to fold: it is kept as it is, which is also what Windows does. A capture with more than two channels is now read frame by frame rather than as if it were stereo. Help, Check my audio sources says whether each source is really in stereo, which a level meter cannot show you. Reported by Tony: Logic Pro arrived on YouTube in mono.",
    "3.5.26": "Your stream key is remembered again. It was filed under your radio station's name, which is the wrong name for it: that can be changed, and loading a saved setup could clear it, and when it was cleared the key was still sitting in the keychain with nothing able to find it. It looked exactly like the app forgetting the key, and the only way back on air was to fetch a fresh one from the platform. Every single time. It is now filed under the platform, so YouTube and Facebook each keep their own and neither can overwrite the other, and a key left under an old station name is carried across the first time it is needed. It is never guessed at: a key the board does not name is left alone, because a guess that moves a key is a guess that can lose one. Reported by Tony, who had to fetch a new key from YouTube before every broadcast.",
    "3.5.25": 'Your extra audio sources really go on the air now. **They never could.** A source set to capture a program was created, reported itself running, showed no error and sent silence, so a broadcast with your screen reader or Logic in it went out with only the microphone and nothing anywhere said so. Core Audio was being asked for the capture in a way it refuses without saying: it answered success and handed back nothing. It is now asked the way that works, measured on the machine rather than guessed at, and both are proven on the air, VoiceOver at minus 25 decibels and Logic Pro at minus 12. Three more faults of the same family went with it. Adding a second source no longer inherits the program the one above it uses, which is how one source ended up pointing at the wrong program: there is a not chosen entry now, and a program a board names is kept in the list even when it is not running, so opening the panel can never throw that choice away. A source whose interface is unplugged says so instead of quietly capturing the built in microphone, which is what Windows has always done. Two copies of Drop Deck no longer stop each other capturing the same program. On top of that, sources that will not open are now said out loud when the app opens and again before you go live rather than being noticed by nobody, one that is not ready is retried every few seconds so a program you start after going on air comes on by itself, and Help now has Check my audio sources, which listens for six seconds and tells you in words whether each one is really putting sound out. It is safe to run mid show. Reported by Tony, who added VoiceOver and Logic and heard neither.',
